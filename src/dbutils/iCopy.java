	...
	public void Write2DB(int limitSize) {
		...
		try {
			...
			for (itable itab : objDBts.objToSchema.gettables()) {
				...
				if (objDBts.isDbtable(objDBts.objToSchema.getName().toLowerCase(), itab.getName().toLowerCase())) {
					...
					if (limitSize > 0) {
						WhereClause = objDBts.objToSchema.getRecuriveFKs(itab, objDBts.objFrmSchema.getName());// get this from from Schema Clause
